...
const GoldPriceDashboard = () => {
  ...
  const fetchPrices = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axios.get("https://goldbackend-production-eaef.up.railway.app/Goldprices/all");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        // PUT Request
        await axios.put(`https://goldbackend-production-eaef.up.railway.app/Goldprices/${editingId}`, payload);
        alert("Record updated successfully!");
      } else {
        // POST Request (Aligned to match standard API route rules)
        await axios.post("https://goldbackend-production-eaef.up.railway.app/Goldprices/add", payload);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to permanently delete this pricing history item?")) {
      try {
        await axios.delete(`https://goldbackend-production-eaef.up.railway.app/Goldprices/delete/${id}`);
